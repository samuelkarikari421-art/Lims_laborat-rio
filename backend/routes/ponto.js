...
async function getUsuarioRequisicao(req) {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader) return null;
        const token = authHeader.split(' ')[1];
        const decoded = jwt.verify(token, SECRET);
        const userRes = await pool.query("SELECT id, nome, perfil, permissoes FROM usuarios WHERE id = $1", [decoded.id]);
...
async function registrarLog(req, acao, detalhes) {
    ...
    const u = await getUsuarioRequisicao(req);
    if (u) usuarioNome = u.nome;
...
// =========================================================================
// 1. LISTAR PONTOS (BLINDADO: SÓ VÊ TODOS QUEM ASSINA)
// =========================================================================
router.get("/", async (req, res) => {
    try {
        const user = await getUsuarioRequisicao(req);
        let whereClause = "";
        let params = [];

        if (user) {
            const isAdmin = (user.perfil === 'ADMIN' || user.perfil === 'ADMINISTRADOR');
            // 🔥 NOVA REGRA: Só vê tudo quem tem poder de Assinar a folha (ponto_sign)
            const podeVerTudo = isAdmin || (user.permissoes && user.permissoes.ponto_sign === true);
...
router.post("/", async (req, res) => {
    try {
        ...
        await pool.query(
            `INSERT INTO controle_ponto (usuario_id, data_registro, hora_entrada, hora_saida, observacoes, tipo_registro) VALUES ($1, $2, $3, $4, $5, $6)`,
...
router.put("/:id", async (req, res) => {
    try {
        const { usuario_id, data_registro, hora_entrada, hora_saida, observacoes, tipo_registro } = req.body;
        await pool.query(
            `UPDATE controle_ponto SET usuario_id = $1, data_registro = $2, hora_entrada = $3, hora_saida = $4, observacoes = $5, tipo_registro = $6 WHERE id = $7`,
    ...
    } catch (err) { res.status(500).json({ error: "Erro ao atualizar ponto" }); }
});
...
router.post("/relatorio/salvar", async (req, res) => {
    try {
        ...
        const pontosRes = await pool.query(`
            SELECT to_char(data_registro, 'DD/MM/YYYY') as data_fmt, to_char(hora_entrada, 'HH24:MI') as entrada, to_char(hora_saida, 'HH24:MI') as saida
            FROM controle_ponto WHERE usuario_id = $1 AND to_char(data_registro, 'YYYY-MM') = $2 ORDER BY data_registro ASC, hora_entrada ASC
        `, [usuario_id, mesano]);
        ...
        doc.text('Data', col1 + 5, y + 5); doc.text('Entrada', col2 + 5, y + 5); doc.text('Saída', col3 + 5, y + 5); doc.text('Horas Trab.', col4 + 5, y + 5);
        y += 20;
        ...
        pontos.forEach(p => {
            ...
            doc.moveTo(col2, y).lineTo(col2, y + 20).stroke(); doc.moveTo(col3, y).lineTo(col3, y + 20).stroke(); doc.moveTo(col4, y).lineTo(col4, y + 20).stroke();
            ...
            if (p.entrada && p.saida) {
                ...
                txtHoras = `${Math.floor(diff / 60)}h ${String(diff % 60).padStart(2, '0')}m`;
            }
            doc.text(p.data_fmt, col1 + 5, y + 5); doc.text(p.entrada || '--:--', col2 + 5, y + 5); doc.text(p.saida || '--:--', col3 + 5, y + 5); doc.text(txtHoras, col4 + 5, y + 5);
            y += 20;
        });
        ...
        doc.text(`${Math.floor(totalMinutosMes / 60)}h ${String(totalMinutosMes % 60).padStart(2, '0')}m`, col4 + 5, y + 5);

        y += 80;
        if (y > 700) { doc.addPage(); y = 50; }
        doc.font('Helvetica').fontSize(10);
        doc.moveTo(80, y).lineTo(250, y).stroke(); doc.text("Assinatura do Colaborador", 80, y + 5, { width: 170, align: 'center' });

        if (responsavel.assinatura) {
            try {
                const imageBuffer = Buffer.from(responsavel.assinatura.split(';base64,').pop(), 'base64');
        ...
        doc.moveTo(320, y).lineTo(490, y).stroke(); doc.text("Supervisor / Responsável", 320, y + 5, { width: 170, align: 'center' });
        doc.fontSize(8).text(responsavel.nome || 'Gestor LIMS', 320, y + 20, { width: 170, align: 'center' });
        doc.end();
    } catch (err) { res.status(500).json({ success: false, message: "Erro ao processar PDF." }); }
});

// =========================================================================
// 6. LISTAR E EXCLUIR FOLHAS ASSINADAS (BLINDADO)
// =========================================================================
router.get("/folhas-assinadas", async (req, res) => {
    try {
        ...
        if (user) {
            const isAdmin = (user.perfil === 'ADMIN' || user.perfil === 'ADMINISTRADOR');
            // 🔥 NOVA REGRA APLICADA AQUI TAMBÉM
            const podeVerTudo = isAdmin || (user.permissoes && user.permissoes.ponto_sign === true);
...
router.delete("/folhas-assinadas/:id", async (req, res) => {
    try {
        ...
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: "Erro ao excluir folha" }); }
});
